...
function refreshdns
{
	...
	if [[ "$ContainsErrors" = true ]]; then
		...
		rndc flush
		echo "RNDC flush complete"
		echo ""

		echo "Reloading Remote Name Daemon Control RNDC..."
		rndc reload
		echo "RNDC reload complete"
		echo ""
		echo "**********************************************"
...
function addDnsEntry
{
	...
	if [[ "$userResponse" == "yes" ]]; then
		...
		if [ "$addReverse" = false ]; then
			echo "SKIPPED - $NEWIPADDRESS was already in reverse db $REVERSEFILE" | tee -a $TMP_SESSION_HISTORY
		else
			sed -i "s/$FINDWHAT/$REPLACEWITH/" $REVERSEFILE
			value=$(grep -c "$NEWIPADDRESS" "$REVERSEFILE")
			if [[ $value -gt 0 ]]; then
				echo "SUCCESS - $NEWIPADDRESS was added to reverse db $REVERSEFILE" | tee -a $TMP_SESSION_HISTORY
				UpdateRequired="\e[1;31myes\e[0m"
			else
				echo "FAILED - $NEWIPADDRESS was NOT added to reverse db $REVERSEFILE" | tee -a $TMP_SESSION_HISTORY
				echo "System will now return to the main menu"
				echo ""
				return 1;
			fi
		...
		if [ "$addForward" = false ]; then
			...
			if [[ $value -gt 0 ]]; then
				echo "SUCCESS - $NEWFQDN was added to forward db $FORWARDFILE" | tee -a $TMP_SESSION_HISTORY
				UpdateRequired="\e[1;31myes\e[0m"
			else
				echo "FAILED - $NEWFQDN was NOT added to forward db $FORWARDFILE" | tee -a $TMP_SESSION_HISTORY
				echo "System will now return to the main menu"
				echo ""
				return 1;
			fi
	...
	echo ""
	if [[ "$userResponse" == "yes" ]]; then
		addNewTrust $NEWIPADDRESS ${FQDNAR[0]}
	fi
	return 0;
}
    ...
    do
		...
		if [[ $value -eq 1 ]]; then
			...
			if [[ $DeleteConfirmation == "YES" ]]; then
				...
				value=$(grep -c "$FINDWHAT" "$CONFFILE")
				if [[ $value -eq 0 ]]; then
					echo "SUCCESS - $FINDWHAT was removed from $CONFFILE" | tee -a $TMP_SESSION_HISTORY
					UpdateRequired="\e[1;31myes\e[0m"
				else
					echo "FAILED - $FINDWHAT was NOT removed from $CONFFILE" | tee -a $TMP_SESSION_HISTORY
				fi
				echo ""
...
function addNewTrust
{
	...
	if [[ "$userResponse" == "yes" ]]; then
		...
		if [[ $value -gt 0 ]]; then
			echo "SUCCESS - $NEWIPADDRESS was added to $CONFFILE" | tee -a $TMP_SESSION_HISTORY
		else
			echo "FAILED - $NEWIPADDRESS was NOT added to $CONFFILE" | tee -a $TMP_SESSION_HISTORY
		fi
		UpdateRequired="\e[1;31myes\e[0m"
	elif [[ "$userResponse" == "no" ]]; then
		echo ""
...
function removeTrust
{
	...
	echo ""
	echo "[ $NEWIPADDRESS ] will be removed from the trusted host group"
	echo -n "Would you like to continue? (YES/no): "
	read -e userResponse
	if [[ "$userResponse" == "YES" ]]; then
		...
		value=$(grep -c "$NEWIPADDRESS" "$CONFFILE")
		if [[ $value -eq 0 ]]; then
			echo "SUCCESS - $NEWIPADDRESS was removed from $CONFFILE" | tee -a $TMP_SESSION_HISTORY
		else
			echo "FAILED - $NEWIPADDRESS was NOT removed from $CONFFILE" | tee -a $TMP_SESSION_HISTORY
		fi
		UpdateRequired="\e[1;31myes\e[0m"
	elif [[ "$userResponse" == "no" ]]; then
		echo ""
...
function togglebindservice
{
	...
	if [[ "$bindStatus" == "0" ]]; then
		...
		echo "SUCCESS - toggled bind9 service to INACTIVE" >> $TMP_SESSION_HISTORY
	else
		systemctl start bind9
		echo "SUCCESS - toggled bind9 service to ACTIVE" >> $TMP_SESSION_HISTORY
	fi
	return 0;
}
...
function creatDnstoolsconffile
{
	if [ ! -f $DNSToolsConfig ]; then
		...
		if [ -f $DNSToolsConfig ]; then
			echo "SUCCESS - Default $DNSToolsConfig was created" | tee -a $TMP_SESSION_HISTORY
			echo ""
			echo "$DNSToolsConfig exists but is not configured. Please configure"
		else
			echo "FAILED - Default $DNSToolsConfig could not be created" | tee -a $TMP_SESSION_HISTORY
			echo ""
